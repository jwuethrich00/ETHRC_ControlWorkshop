...
# initial condition
inverted_pendulum.state = np.array([np.deg2rad(0.0), 0.0])
cart_pole_sys.state = np.array([0, 0, np.deg2rad(2.0), 0.0])

if controller_test == "No Control":
    simulate_no_controller(cart_pole_sys, T_sim, dt, "Nonlinear") #inverted_pendulum or cart_pole_sys,"Nonlinear" or "Linear"
    ...
    simulate_LinearMPC(cart_pole_sys, "reference", N_hor=15, T_sim=10, dt=0.005) #N_hor = 5 or 15 -> approx. same for stabilize, see difference for reference
elif controller_test == "Nonlinear MPC":
    simulate_NonlinearMPC(cart_pole_sys, N_hor=100, T_sim = 6, dt = 0.01)
else:
