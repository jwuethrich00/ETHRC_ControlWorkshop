...
def simulate_and_estimate(Q, R, label):
    ...
    for t in range(1, T):
        ...
        # EKF
        x_pred_ekf = f(x_ekf[:, t-1], u[t-1])
        F = F_jacobian(x_ekf[:, t-1], u[t-1])
        P_ekf = F @ P_ekf @ F.T + Q
        H = H_jacobian(x_pred_ekf)
        y_pred = h(x_pred_ekf)
        S_ekf = H @ P_ekf @ H.T + R
        K_ekf = P_ekf @ H.T @ np.linalg.inv(S_ekf)
        x_ekf[:, t] = x_pred_ekf + K_ekf.flatten() * (y[t] - y_pred)
        P_ekf = (np.eye(2) - K_ekf @ H) @ P_ekf
